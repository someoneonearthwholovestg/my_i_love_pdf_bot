...
async def _color(bot, callbackQuery):
    try:
        ...
        if downloadLoc is None:
            ...
            return
        #STAMPING STARTED
        await downloadMessage.edit("`Started Stamping..` 💠")
        ...
        await callbackQuery.message.reply_document(
            file_name=f"{fileNm}.pdf", document=open(output_file, "rb"),
            thumb=PDF_THUMBNAIL, quote=True, caption="stamped pdf"
        )
        # DELETE DOWNLOAD MESSAGE
        await downloadMessage.delete(); PROCESS.remove(chat_id); shutil.rmtree(f"{message_id}")
    except Exception as e:
        try:
            print("Stamp: ", e); PROCESS.remove(chat_id)
            shutil.rmtree(f"{message_id}"); await downloadMessage.delete()
        except Exception:
            pass
